...
def subject_create(request):
    ...
    return JsonResponse({'id': str(subject.id), 'name': subject.name, 'code': subject.code,
...
def teacher_create(request):
    ...
    return JsonResponse({
        ...
        'subject_ids': [str(sid) for sid in teacher.subjects.values_list('id', flat=True)],
    })
...
def classroom_create(request):
    ...
    return JsonResponse({
        'id': str(classroom.id), 'room_name': classroom.room_name,
...
def class_create(request):
    ...
    return JsonResponse({
        'id': str(school_class.id), 'class_name': school_class.class_name,
        'grade_level': school_class.grade_level, 'section': school_class.section,
        'class_teacher_id': str(school_class.class_teacher_id) if school_class.class_teacher_id else None,
    })
...
def period_create(request):
    ...
    return JsonResponse({
        'id': str(period.id), 'label': period.label,
...
def timetable_generate(request, pk):
    timetable = get_object_or_404(Timetable, pk=pk)
    outcome = generate_timetable(str(timetable.id))
...
def timetable_conflicts(request, pk):
    timetable = get_object_or_404(Timetable, pk=pk)
    conflicts = detect_conflicts(str(timetable.id))
...
def api_teachers(request):
    ...
    return JsonResponse([{
        ...
        'subject_ids': [str(sid) for sid in t.subjects.values_list('id', flat=True)],
...
def api_classes(request):
    ...
    for c in classes:
        c['id'] = str(c['id'])
        c['class_teacher_id'] = str(c['class_teacher_id']) if c['class_teacher_id'] else None
...
def api_periods(request):
    ...
    # Convert time fields to string and UUID to string
    for p in periods:
        p['id'] = str(p['id'])
...
@login_required
@require_http_methods(["POST"])
def import_from_academics(request):
    """Import subjects, teachers, and classes from the academics app"""
    school = get_current_school()
    ...
    try:
        ...
        return JsonResponse({'success': False, 'error': 'Academics app not available'}, status=400)
    ...
    return JsonResponse({
        ...
        'message': f'Imported {subjects_created} subjects, {teachers_created} teachers, {classes_created} classes'
    })
